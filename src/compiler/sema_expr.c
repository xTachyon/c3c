...
{
	expr->type = left->type;
	if (right->type->type_kind == TYPE_FAILABLE && expr->type->type_kind != TYPE_FAILABLE)
	{
		expr->type = type_get_failable(expr->type);
	}
...
static inline bool sema_cast_ident_rvalue(Context *context, Type *to, Expr *expr)
{
	...
	switch (decl->var.kind)
	{
		case VARDECL_CONST:
			...
			return sema_analyse_inferred_expr(context, to, expr);
		case VARDECL_PARAM_EXPR:
			expr_replace(expr, copy_expr(decl->var.init_expr));
		...
		switch (decl->var.kind)
		{
			case VARDECL_CONST:
				if (!decl->type)
				{
					...
					if (!sema_analyse_inferred_expr(context, to, copy)) return false;
...
static bool sema_expr_analyse_macro_call(Context *context, Expr *call_expr, Expr *struct_var, Decl *decl, bool failable)
{
	...
	SCOPE_OUTER_START
		...
		context->expected_block_type = foo ? foo->type : NULL;
		SCOPE_START_WITH_FLAGS(SCOPE_MACRO);
			...
			{
				if (context->expected_block_type && context->expected_block_type != type_void)
				{
					SEMA_ERROR(decl, "Missing return in macro that evaluates to %s.", type_to_error_string(context->expected_block_type));
					ok = false;
					goto EXIT;
				}
			}
...
static bool sema_analyse_body_expansion(Context *context, Expr *call)
{
	...
	for (unsigned i = 0; i < expressions; i++)
	{
		...
		if (!sema_analyse_inferred_expr(context, param->type, expr)) return false;
	}
...
static inline bool sema_expr_analyse_cast(Context *context, Expr *expr)
{
	...
	success &= sema_analyse_expr(context, inner);
...
{
	if (!sema_analyse_expr(context, left) || !sema_analyse_expr(context, right)) return false;
...
static bool sema_expr_analyse_comp(Context *context, Expr *expr, Expr *left, Expr *right)
{
	...
	if (!cast_implicit_ignore_failable(left, max)) goto ERR;
	if (!cast_implicit_ignore_failable(right, max)) goto ERR;
DONE:
	...
	bool is_failable = IS_FAILABLE(left) || IS_FAILABLE(right);

	Type *return_type = left_type->type_kind == TYPE_VECTOR ? type_get_vector_bool(left_type) : type_bool;

	expr->type = type_get_opt_fail(return_type, is_failable);
...
static bool sema_expr_analyse_not(Expr *expr, Expr *inner)
{
	...
	if (!cast_implicit(inner, expr->type)) return false;
...
static inline bool sema_expr_analyse_taddr(Context *context, Type *to, Expr *expr, Expr *inner)
{
	...
	if (!sema_analyse_inferred_expr(context, inferred_type, inner)) return false;
...
static inline bool sema_expr_analyse_unary(Context *context, Type *to, Expr *expr)
{
	...
	switch (expr->unary_expr.operator)
	{
		...
		case UNARYOP_NEG:
			if (!sema_analyse_expr(context, inner)) return false;
			return sema_expr_analyse_neg(context, expr, inner);
		case UNARYOP_BITNEG:
			if (!sema_analyse_expr(context, inner)) return false;
			return sema_expr_analyse_bit_not(context, expr, inner);
		case UNARYOP_NOT:
			if (!sema_analyse_expr(context, inner)) return false;
...
static inline bool sema_expr_analyse_guard(Context *context, Type *infer_type, Expr *expr)
{
	Expr *inner = expr->guard_expr.inner;
	if (!sema_analyse_inferred_expr(context, infer_type, inner)) return false;
...
static inline bool sema_expr_analyse_const(Expr *expr)
{
	expr->pure = true;
	return true;
}
...
static inline bool sema_expr_analyse_expr_block(Context *context, Type *assign_to, Expr *expr)
{
	bool success = true;
	...
	Ast **saved_returns = context_push_returns(context);
	context->expected_block_type = assign_to;
	...
	SCOPE_START_WITH_FLAGS(SCOPE_EXPR_BLOCK)
		...
		if (!context->active_scope.jump_end && assign_to)
		{
			SEMA_ERROR(expr, "Expected the block to return with a value of type %s.", type_to_error_string(assign_to));
			success = false;
		}
...
static inline bool sema_expr_analyse_failable(Context *context, Type *to, Expr *expr)
{
	...
	expr->type = to ? type_get_failable(to) : type_anyfail;
	return true;
}
...
static inline bool sema_expr_analyse_decl(Context *context, Expr *expr)
{
...
static inline bool sema_analyse_expr_dispatch(Context *context, Type *infer_type, Expr *expr)
{
	switch (expr->expr_kind)
	{
		case EXPR_COND:
		...
		case EXPR_CATCH_UNWRAP:
			UNREACHABLE
		case EXPR_DECL:
			return sema_expr_analyse_decl(context, expr);
		case EXPR_CT_CALL:
			return sema_expr_analyse_ct_call(context, expr);
		case EXPR_HASH_IDENT:
			return sema_expr_analyse_hash_identifier(context, infer_type, expr);
		case EXPR_CT_IDENT:
			return sema_expr_analyse_ct_identifier(context, expr);
		case EXPR_FAILABLE:
			return sema_expr_analyse_failable(context, infer_type, expr);
		case EXPR_PLACEHOLDER:
		...
		case EXPR_EXPR_BLOCK:
			return sema_expr_analyse_expr_block(context, infer_type, expr);
		case EXPR_GUARD:
			return sema_expr_analyse_guard(context, infer_type, expr);
		case EXPR_CONST:
		...
		case EXPR_CONST_IDENTIFIER:
		case EXPR_IDENTIFIER:
			return sema_expr_analyse_identifier(context, infer_type, expr);
		case EXPR_CALL:
			return sema_expr_analyse_call(context, infer_type, expr);
		case EXPR_SUBSCRIPT:
		...
		case EXPR_INITIALIZER_LIST:
		case EXPR_DESIGNATED_INITIALIZER_LIST:
			return sema_expr_analyse_initializer_list(context, infer_type, expr);
		case EXPR_CAST:
			return sema_expr_analyse_cast(context, expr);
		case EXPR_EXPRESSION_LIST:
			return sema_expr_analyse_expr_list(context, infer_type, expr);
	}
	UNREACHABLE
}
...
{
	return sema_analyse_expr_value(context, NULL, expr);
}
...
{
	return sema_analyse_expr_value(context, NULL, expr) && sema_cast_rvalue(context, NULL, expr);
}

bool sema_analyse_inferred_expr(Context *context, Type *infer_type, Expr *expr)
{
	return sema_analyse_expr_value(context, infer_type, expr) && sema_cast_rvalue(context, infer_type, expr);
}
