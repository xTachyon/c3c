...
{
	if (d == 0) return (Int128) { 0, 0 };
	if (d < 0)
	{
		if (d >= INT64_MIN) return (Int128) { UINT64_MAX, (int64_t)d };
		d = -d;
		Real upper = d / U64F;
		if (upper > (1.0 + (Real)INT64_MAX)) return (Int128) { INT64_MIN, 0 };
		uint64_t top = (uint64_t)upper;
		Real lower = d - upper * U64F;
		uint64_t bottom = (uint64_t)lower;
		return (Int128) { ~top, ~bottom };
	}
	if (d <= INT64_MAX) return (Int128) { 0, (int64_t)d };
	Real upper = d / U64F;
	if (upper > (Real)INT64_MAX) return (Int128) { INT64_MAX, UINT64_MAX };
	uint64_t top = (uint64_t)upper;
	Real lower = d - upper * U64F;
	uint64_t bottom = (uint64_t)lower;
	return (Int128) { top, bottom };
...
{
	if (d <= 0) return (Int128) { 0, 0 };
	if (d <= UINT64_MAX) return (Int128) { 0, (uint64_t)d };
	Real upper = d / U64F;
	if (upper > (Real)UINT64_MAX) return (Int128) { UINT64_MAX, UINT64_MAX };
	uint64_t top = (uint64_t)upper;
	Real lower = d - upper * U64F;
	uint64_t bottom = (uint64_t)lower;
	return (Int128) { top, bottom };
